...
def search(request):
    ...
    if allPosts.count() == 0:
        messages.warning(request, 'Please enter keyword correctly!')
...
def handleSignup(request):
    if request.method == 'POST':
        ...
        # Check for errorneos inputs
        if len(username) > 10:
            messages.error(request, "Username must under 10 characters")
            return redirect('home')

        if not username.isalnum():
            messages.error(request, "Username should only contain letters and characters")
            return redirect('home')
